  ...
  return (
    <Link 
     ...
     href={`/travel-packages/${product.periodo}/${product.ID}`}>
                <img className='w-full transition-all group-hover:scale-110' src={product.image} alt={product.destino} />
                <p className='pl-5 text-2xl font-black tracking-wide w-max pt-7 text-tertiary'>{product.destino}</p>
                ...
                <div className='flex flex-col justify-center w-3/4 gap-8'>
                    ...
                    <small className='pl-5 text-sm text-gray-600 '>{product.descripcion}</small>
                    <b className='pl-5 font-semibold text-md'>Salida: {product.date}</b>
                    <div className='flex flex-col items-center w-full'>
                        {product.desde === 1 && <small className='text-xs'>desde</small>}
                        <p className='md:text-[40px] text-4xl font-bold text-tertiary'>${product.precio}.-</p>
